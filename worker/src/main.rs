...
use worker::{process_once_dual, WorkerConfig, WorkerCycleOutcome};
...
async fn main() -> Result<()> {
    ...
    info!(
        ...
        llm_local_small_configured = config.llm.local_small.is_some(),
        llm_local_interactive_tier = config.llm.local_interactive_tier.as_str(),
        llm_local_batch_tier = config.llm.local_batch_tier.as_str(),
...
async fn run_forever(pool: &DbPool, config: &WorkerConfig) -> Result<()> {
    loop {
        match process_once_dual(pool, config).await? {
