...
echo "[capture-baseline] api=${API_BASE_URL} tenant=${TENANT_ID} role=${USER_ROLE} window_secs=${WINDOW_SECS} trace_limit=${TRACE_LIMIT}"
...
TRACES_PATH="${OUTPUT_DIR}/${BASELINE_PREFIX}_latency_traces.json"
echo "[capture-baseline] summary=${SUMMARY_PATH}"
echo "[capture-baseline] histogram=${HISTOGRAM_PATH}"
echo "[capture-baseline] traces=${TRACES_PATH}"
