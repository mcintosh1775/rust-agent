...
RUN_LLM_SMOKE="${RELEASE_GATE_RUN_LLM_SMOKE:-0}"
...
if [[ "${RUN_LLM_SMOKE}" == "1" ]]; then
  if [[ -z "${RELEASE_SMOKE_DB_PATH}" ]]; then
    echo "[release-gate] RELEASE_SMOKE_DB_PATH is required for llm smoke"
    exit 1
  fi
  echo "[release-gate] llm smoke check"
  make -C "${REPO_ROOT}" \
    DB="${RELEASE_SMOKE_DB_PATH}" \
    RELEASE_SMOKE_TENANT_ID="${RELEASE_GATE_TENANT_ID:-single}" \
    RELEASE_LLM_SMOKE_RECIPE_ID="${RELEASE_GATE_LLM_SMOKE_RECIPE_ID:-llm_remote_v1}" \
    RELEASE_LLM_SMOKE_EXPECTED_ROUTE="${RELEASE_GATE_LLM_SMOKE_EXPECTED_ROUTE:-remote}" \
    ${RELEASE_GATE_LLM_SMOKE_EXPECTED_HOST:+RELEASE_LLM_SMOKE_EXPECTED_HOST="${RELEASE_GATE_LLM_SMOKE_EXPECTED_HOST}"} \
    ${RELEASE_GATE_LLM_SMOKE_EXPECTED_MODEL:+RELEASE_LLM_SMOKE_EXPECTED_MODEL="${RELEASE_GATE_LLM_SMOKE_EXPECTED_MODEL}"} \
    release-llm-smoke
fi
