...
fn console_index_route_serves_html_shell() -> Result<(), Box<dyn std::error::Error>> {
    run_async(async {
        ...
        assert!(body_text.contains("Load Bootstrap"));
        assert!(body_text.contains("Complete Bootstrap"));
        assert!(body_text.contains("/v1/agents/:id/bootstrap"));
        assert!(body_text.contains("bootstrap-note"));
